...
public class Matrix implements Serializable {
	...
	public Matrix(float[] inputData, int rows, int cols) {
	...
	public static float[][] dataCopy(float[][] prevData){
		...
		float[][] result = new float[prevData.length][prevData[0].length];
	...
	public Matrix randomize(float deviation) {
		
		for (int i = 0; i < getRows(); i++)
			for (int j = 0; j < getCols(); j++)
				set(i, j, (float) rand.nextGaussian() * deviation);
		return this;
	}
	
	public Matrix randomize(float min, float max) {
		
		for (int i = 0; i < getRows(); i++)
			for (int j = 0; j < getCols(); j++)
				set(i, j, (float) rand.nextGaussian() * (max - min) + min);
		return this;
	}
	...
	public static Matrix add(Matrix mat1, Matrix mat2) {
		...
		float[][] resultData = new float[mat1.getRows()][mat1.getCols()];
	...
	public static Matrix add(Matrix mat, float scalar) {
		...
		float[][] resultData = new float[mat.getRows()][mat.getCols()];
	...
	public static Matrix sub(Matrix mat, float scalar) {
	...
	public float dot(Matrix mat) {
		
		if (getRows() != mat.getRows() || getCols() != mat.getCols())
			throw new RuntimeException("Matrix dimensions mismatch in dynamic method: float dot(Matrix mat): ("
	...
	public Matrix mult(Matrix mat) {
		...
		float[][] resultData = new float[getRows()][mat.getCols()];
		for (int i = 0; i < getRows(); i++)
			for (int j = 0; j < mat.getCols(); j++) {
				resultData[i][j] = 0.0f;
	...
	public static Matrix mult(Matrix mat1, Matrix mat2) {
		...
		float[][] resultData = new float[mat1.getRows()][mat2.getCols()];
		for (int i = 0; i < mat1.getRows(); i++)
	...
	public static Matrix mult(Matrix mat, float scalar) {
	...
	public static Matrix div(Matrix mat, float scalar) {
	...
	public Matrix kronecker(Matrix mat) {
		...
		float[][] resultData = new float[getRows() * mat.getRows()][getCols() * mat.getCols()];
		for (int i = 0; i < getRows(); i++)
			for (int j = 0; j < getCols(); j++)
	...
	public static Matrix kronecker(Matrix mat1, Matrix mat2) {
		...
		float[][] resultData = new float[mat1.getRows() * mat2.getRows()][mat1.getCols() * mat2.getCols()];
		for (int i = 0; i < mat1.getRows(); i++)
			for (int j = 0; j < mat1.getCols(); j++)
	...
	public float determinant() {
		...
		float result = 0;
		for (int i = 0; i < n; i++) {
			float [][] auxData = new float[n - 1][n - 1];
	...
	public Matrix[] decomposeLU() {
		...
		int n = getRows();
		float[][] resultL = new float[n][n];
		float[][] resultU = new float[n][n];
	...
	public static Vector solveLU(Matrix mat, Vector b) {
		...
		float[] y = Vector.dataCopy(b.get());
		for (int i = 0; i < n; i++)
			for (int j = 0; j < i; j++)
				y[i] -= L.get(i, j) * y[j];
		
		float[] x = Vector.dataCopy(y);
		for (int i = n - 1; i >= 0; i--) {
			for (int j = n - 1; j > i; j--)
				x[i] -= U.get(i, j) * x[j];
	...
	public static Vector jacobi(Matrix mat, Vector b, float eps) {
	...
	public static Vector gaussSeidel(Matrix mat, Vector b, float eps) {
	...
	public float maxRowNormSq() {
		
		float[] norms = new float[getRows()];
		for (int i = 0; i < getRows(); i++)
	...
	public float maxColNormSq() {
		
		float[] norms = new float[getCols()];
	...
	public float maxRowNorm() {
		
		return (float) Math.sqrt(maxRowNormSq());
	}
	
	public float maxColNorm() {
		
		return (float) Math.sqrt(maxColNormSq());
	}
	
	public float superNormSq() {
		
		return this.flatten().normSq();
	}
	
	public float superNorm() {
	...
	public Matrix transpose() {
		...
		float[][] resultData = new float[getCols()][getRows()];
		for (int i = 0; i < getCols(); i++)
	...
	public static Matrix transpose(Matrix mat) {
		...
		float[][] resultData = new float[mat.getCols()][mat.getRows()];
	...
	public Vector flatten() {
		...
		float[] resultData = new float[getRows() * getCols()];
		for (int i = 0; i < getRows(); i++)
			for (int j = 0; j < getCols(); j++)
	...
	public static Matrix rotate2D(float angle) {
	...
	public Matrix(float[][] data) {set(data);}
	...
	public Matrix getPortion(int startX, int startY, int deltaX, int deltaY) {
		...
		float[][] resultData = new float[deltaX][deltaY];
	...
	public Matrix set(int i, int j, float scalar) {
		
		if (i >= getRows() && j >= getCols())
			throw new RuntimeException("Index exceeds matrix dimensions in Matrix set(int i, int j, float scalar): (" 
					+ i + "," + j + ") -> (" + getRows() + "," + getCols() + ")");
		data[i][j] = scalar;
		return this;
	}
	
	public Matrix set(float[][] input) {
		
		data = input; rows = input.length; cols = input[0].length;
		return this;
	}
	
	public Matrix setRow(float[] input, int i) {
		
		if (input.length != getCols() || i >= getRows())
			throw new RuntimeException("Dimensions mismatch in Matrix setRow(float[] input, int i): " 
	...
	public Matrix setCol(float[] input, int j) {
		
		if (input.length == getRows() && j < getCols())
			throw new RuntimeException("Dimensions mismatch in Matrix setCol(float[] input, int j): " 
	...
	public Matrix setAll(float scalar) {
	...
	public Matrix setAllBut(int x, int y, float scalar) {
	...
	public Matrix setAsProbable(float scalar, float rate) {
	...
	public static Matrix setAsProbable(Matrix mat, float scalar, float rate) {
	...
	public Vector fromCol(int j) {
		...
		float[] result = new float[getRows()];
		for (int i = 0; i < getRows(); i++)
			result[i] = get(i, j);
